...
> **Version**: v2.242.1 | **Total**: 60+ major features, ~285 capabilities
>
...
| **Git Deploy** | Push-to-deploy, blue-green (auto-refused whenever persistent volumes are declared), Nixpacks (30+ langs), preview envs (never inherit volumes, GitHub commit status posted at every outcome — pending/success/failure), one-time scheduled deploys, two-person deploy approval, HTTPS via Let's Encrypt or a registered certificate, persistent volumes (container-path only, host side managed, existing writable-layer data migrated on the deploy that adds one), **AI-assisted failure diagnosis (BYO API key: Anthropic/OpenAI/Gemini/Grok, default off, manual per-failure trigger, output redacted for secrets before it leaves the box)** | `routes/git_deploys.rs`, `services/ai_diagnosis.rs` | `git_build.rs` | `GitDeploys.tsx` | `git_deploys`, `git_deploy_history`, `git_previews`, `deploy_approvals` |
...
| **Migration Wizard** | Import from cPanel/HestiaCP — sites and databases. Mail accounts are listed (cPanel only), not migrated. Plesk (beta). Backup archive can be fetched by URL (SSRF-guarded, size-capped) instead of requiring an SFTP upload first | `routes/migration.rs` | `migration.rs` | `Migration.tsx` | `migrations` |
...
| **API Keys** | Key CRUD and rotation. `dp_`-prefixed keys authenticate requests since v2.221.0; see §Withdrawn Claims for the `dpiac_`/`dpx_` families, which still do not | `routes/api_keys.rs`, `auth.rs` | (in Settings) | `api_keys` |
...
| **Auto-Optimization** | PHP-FPM worker analysis, nginx workers vs CPUs, memory/disk recommendations | proxied to agent | `diagnostics.rs` | `Diagnostics.tsx` |
...
| **`client` role manages "containers"** | `docs/guides/roles-and-ownership.md`, `migrations/20260805000000_client_role.sql` | Administrator-only and always was. Every handler in `routes/docker_apps.rs` calls `require_admin`, and the dashboard's "Deploy App" shortcut pointed a client at a page that bounces them. The other five items in that list — PHP version, settings, files, backups, SSL — are real and ownership-scoped via `SITE_CALLER_PREDICATE`. Reported by the operator the role was built for, on #51. **The claim also survives in the migration's own comment, which cannot be corrected**: `sqlx::migrate!` checksums applied migrations, so editing that file would break the upgrade on every deployed install. It is left in place deliberately and recorded here instead. ⚠ **The handler figure in this row was itself wrong until s334**: it read "42 of 42", a number obtained by counting `AdminUser` *occurrences* — which includes the `use` import — rather than counting handlers. `mail.rs` has had **41** `pub async fn` at every tag from v2.83.0 to v2.91.0, checked one by one. The v2.83.0 CHANGELOG entry still carries the old figure and is left as shipped, since a released changelog is a record of what was said. **Mail withdrawn from this row at v2.102.0/v2.103.0**: `routes/mail.rs` now takes `AuthUser` on 9 handlers (list/create/update/delete for mailboxes and aliases, plus DNS lookup) scoped to a domain matching a site the client owns, and `Mail.tsx`'s admin-only redirect was removed — a client manages mail on their own domain exactly as `docs/guides/roles-and-ownership.md` describes. This row was left claiming the opposite for many releases after the reversal shipped (v2.102.0/v2.103.0, well over a hundred releases back); corrected here rather than deleted, per this table's own stated policy. | 2026-08-06, corrected 2026-09-05 |
...
| **Cloudflare Tunnel** — "token-based config, systemd service" | `README.md`, `FEATURES.md` | The installer half is real: `routes/system.rs` installs and uninstalls the `cloudflared` package. The rest is unreachable. `POST /api/tunnel/configure` and `GET /api/tunnel/status` live in `routes/dns.rs` — not `routes/system.rs`, which is what the row named — and **have no caller anywhere in the SPA, the CLI or the docs**; the `cloudflared` row in `Settings.tsx` renders `extra: () => null`, and the installer sends a body for `powerdns` only, so there is no field through which a tunnel token can reach the panel. `configure_tunnel` is also the only writer of the systemd unit, so the installed binary can never be started: `tunnel_status` answers `running:false, configured:false` permanently. The settings key `tunnel_configured` is written by that one unreachable handler and **read by nothing**. Withdrawn rather than built, because supplying the token is a product decision about where an operator's Cloudflare credential should live. **The dead code itself was removed at v2.231.0** — `configure_tunnel`/`tunnel_status` and the `tunnel_configured` settings write no longer exist; only the installer half (this table's Integrations row above) remains. | 2026-08-16, code removed 2026-09-07 |
| **147 one-click app templates** — all of them working | `README.md`, `FEATURES.md`, dockpanel.dev, the Apps page | The catalogue had **never been run**. Deploying all 148 for the first time at v2.113.0 found **15 that need nothing from the operator and still cannot start or serve**. Six were fixed in that release; **eight remain broken**. Six are recorded in `CENSUS_KNOWN_BROKEN` in `docker_apps.rs`, each naming what is wrong; **drone** and **azuracast** are recorded here instead, because both also require a value the operator must type, which means the weekly census cannot support a claim about them in either direction: vector and garage need a config their images do not ship, erpnext needs a bench site no environment value creates, azuracast has a third startup failure behind two that were fixed, drone needs SCM credentials the form never asks for, authelia cannot be given a start command without turning a loud failure into a silent one, flowise is broken upstream on the tag pinned, and wazuh is one container of a three-container stack. A further **34 require an external service the operator supplies**, so they are unproven in either direction rather than broken. **Two templates were withdrawn outright rather than left broken-and-documented (v2.178.0, 148 → 146)**: **dozzle** and **portainer** both needed the host Docker socket for their core purpose, and `deploy_app` deliberately never mounts it into any template's own container — the same policy the Compose-stack path enforces too (`routes/mod.rs` and `compose.rs` both independently block a socket mount, plus host-root and a few other sensitive paths), because it is a full host-escape vector regardless of who is allowed to trigger the deploy. **Dozzle was reinstated at v2.179.0** (146 → 147) behind a `docker-socket-proxy` sidecar (`services::app_sidecar`) that holds the real socket instead, exposing only a `CONTAINERS`-scoped read-only slice of the Docker API — a read-only proxy is sufficient because listing containers and reading logs is all Dozzle ever needed. **Portainer stays withdrawn permanently**: its core purpose (create/stop/exec containers, manage volumes/networks) needs write-level API access a safe proxy would either gut it to near-nothing (read-only) or reopen most of the original host-escape risk (write-permitting) — and it is largely redundant with DockPanel's own native Docker Apps management page. The weekly `Template Census` deploys every template and reports, so this figure cannot go stale again without something going red. | 2026-08-29 |
...
| **Subscription billing** — Stripe Checkout + Customer Portal, no public claim beyond the code itself | (never surfaced in README/FEATURES/docs — see note) | The now-deleted routes/billing.rs was a complete, actively-hardened Stripe integration (Checkout session creation, Customer Portal, a signature-verified webhook) with **zero UI trigger anywhere** — no Subscribe button, no Manage Billing link. Even reachable, it sold nothing: migration `20260313100000_remove_billing_enforcement.sql` had already set `plan_server_limit` unconditionally unlimited for every user, so a completed checkout changed nothing an account could do. This sat in direct contradiction with the product's own stated identity (README: "Zero subscriptions", "no premium tier and nothing is held back") and was never itself listed as a feature anywhere. **Withdrawn rather than wired up** (2026-09-06): routes, config (`STRIPE_SECRET_KEY`/`STRIPE_WEBHOOK_SECRET`), the `stripe_price_*` settings and their Settings.tsx panel, and the now-inert `users.stripe_customer_id`/`stripe_subscription_id`/`plan`/`plan_status`/`plan_server_limit` columns are all removed. Found by the first-ever Loose-Ends Audit (v2.223.0), re-verified against current HEAD before withdrawal. | 2026-09-06 |
| **Restic incremental backups** — "Restic incremental (API only), one-click restore", "encrypted, deduplicated" | `README.md` | Real, working, JWT-authenticated code end-to-end (`POST /api/sites/{id}/restic/backup`, `GET .../restic/snapshots`, `POST .../restic/restore/{snapshot_id}`, proxying to three agent-side handlers) — but it never got any of the reliability treatment local backups get: no scheduler entry, no retry ladder, no failure alert, no staleness sweep, and no retention (`forget`/`prune` were never implemented, so snapshots accumulated on disk forever). It was also never reachable from anywhere a self-hosted operator would find it — no frontend control (one cosmetic keyword in the command palette's search index, no page), no CLI subcommand, and absent from `docs/api-reference.md` despite the README calling it a real, supported "API only" capability. A three-session-old deferred finding (first flagged in the original Loose-Ends Audit, v2.223.0, re-confirmed "zero exposure" at v2.227.0/v2.228.0 without ever being put to an actual decision) was finally scoped as build-vs-withdraw: building parity would have been cheap (the codebase already has this exact scheduler pattern twice) but would still have left it headless and undemanded, while the real risk was never the code — it was the README selling a working, alertless, retention-less backup path as a supported feature to anyone who read it and called the API directly. **Withdrawn rather than built** (2026-09-06): all three endpoints, the agent-side Restic operations, the rename-carry entry for its on-disk repo path, and the two README lines are removed. No DB migration was needed — the feature never had any persisted schema (its password lived only in a flat file on the agent's disk). | 2026-09-06 |
