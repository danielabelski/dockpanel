...
<p align="center">
  ...
  Self-hosted. Docker-native. Written in Rust. Panel services run on <strong>~49MB of RAM</strong>. 831 HTTP routes. 147 app templates. 4426 regression assertions. ~49MB binaries. Zero subscriptions.
</p>
...
**3 Rust binaries**: Agent (~21MB), API (~24MB), CLI (~3.9MB). Runtime RAM: ~35MB agent + ~14MB API ≈ 49MB for the panel itself; ~109MB with the bundled PostgreSQL. 16 supervised background services.
